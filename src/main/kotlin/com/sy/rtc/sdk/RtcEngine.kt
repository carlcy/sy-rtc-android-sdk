...
class RtcEngine private constructor() {
    ...
    private fun sendRoomAction(action: String, data: Map<String, Any?>) {
        val msg = org.json.JSONObject(mapOf(
            "_sy_type" to "room-msg",
            "action" to action,
            "data" to org.json.JSONObject(data)
        )).toString()
    ...
    fun updateRoomInfo(roomInfo: Map<String, Any?>) {
        sendRoomAction("room-info-update", mapOf("roomInfo" to roomInfo))
    }

    fun setRoomNotice(notice: String) {
        sendRoomAction("room-notice-update", mapOf("notice" to notice))
    }

    fun setRoomManager(uid: String, isManager: Boolean) {
        sendRoomAction("manager-update", mapOf("uid" to uid, "isManager" to isManager))
    }

    // ==================== Seat management ====================

    fun takeSeat(seatIndex: Int) {
        sendRoomAction("seat-take", mapOf("seatIndex" to seatIndex))
    }

    fun leaveSeat() {
        sendRoomAction("seat-leave", emptyMap())
    }

    fun requestSeat(seatIndex: Int? = null) {
        sendRoomAction("seat-request", mapOf("seatIndex" to seatIndex))
    }

    fun cancelSeatRequest() {
        sendRoomAction("seat-request-cancel", emptyMap())
    }

    fun handleSeatRequest(uid: String, approved: Boolean, seatIndex: Int? = null) {
        sendRoomAction("seat-request-handle", mapOf("uid" to uid, "approved" to approved, "seatIndex" to seatIndex))
    }

    fun inviteToSeat(uid: String, seatIndex: Int) {
        sendRoomAction("seat-invite", mapOf("uid" to uid, "seatIndex" to seatIndex))
    }

    fun handleSeatInvitation(accepted: Boolean, seatIndex: Int) {
        sendRoomAction("seat-invite-handle", mapOf("accepted" to accepted, "seatIndex" to seatIndex))
    }

    fun kickFromSeat(uid: String) {
        sendRoomAction("seat-kick", mapOf("uid" to uid))
    }

    fun lockSeat(seatIndex: Int) {
        sendRoomAction("seat-lock", mapOf("seatIndex" to seatIndex, "locked" to true))
    }

    fun unlockSeat(seatIndex: Int) {
        sendRoomAction("seat-lock", mapOf("seatIndex" to seatIndex, "locked" to false))
    }

    fun muteSeat(seatIndex: Int) {
        sendRoomAction("seat-mute", mapOf("seatIndex" to seatIndex, "muted" to true))
    }

    fun unmuteSeat(seatIndex: Int) {
        sendRoomAction("seat-mute", mapOf("seatIndex" to seatIndex, "muted" to false))
    }

    // ==================== User management ====================

    fun kickUser(uid: String) {
        sendRoomAction("user-kick", mapOf("uid" to uid))
    }

    fun muteUser(uid: String, muted: Boolean) {
        sendRoomAction("user-mute", mapOf("uid" to uid, "muted" to muted))
    }

    fun banUser(uid: String, banned: Boolean) {
        sendRoomAction("user-ban", mapOf("uid" to uid, "banned" to banned))
    }

    // ==================== Chat & Gift ====================

    fun sendRoomMessage(content: String, messageType: String = "text", extra: Map<String, Any?>? = null) {
        sendRoomAction("room-chat", mapOf("messageType" to messageType, "content" to content, "extra" to extra))
    }

    fun sendGift(giftId: String, toUid: String, count: Int, extra: Map<String, Any?>? = null) {
        sendRoomAction("gift", mapOf("giftId" to giftId, "toUid" to toUid, "count" to count, "extra" to extra))
    }
